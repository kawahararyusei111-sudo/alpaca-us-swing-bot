...
- 監視銘柄: MU, NVDA, MSFT, PLTR, CRWV, NBIS, QQQ, AAPL, SPY, JNJ, JPM（固定・裁量判断なし）
  - JNJ（ヘルスケア）・JPM（金融）は、値動きの大きいハイテク・半導体銘柄群に対するリスク分散のために追加
- 1時間足でSMA20とSMA50を計算
  - ゴールデンクロス（SMA20がSMA50を上抜け）→ 未保有かつ保有銘柄数が3未満なら、資産の30%相当を買い
...
- 1銘柄あたりの投資上限は資産の30%、同時保有は最大3銘柄まで（理論上、最大で資産の90%まで投資される）。
- 含み損-5%で強制損切り。
